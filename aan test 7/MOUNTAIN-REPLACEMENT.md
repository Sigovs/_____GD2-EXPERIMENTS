...
## 16. Abyss transition (structural test, no canyon video yet)
`src/abyss-transition.js` (`ABYSS_TRANSITION` + `createAbyssTransition()`), hooked in `mountain.js`. The page is now **13 viewports**: the first 9/13 (`descentShare`) carry the descent exactly as before (same pixels of scroll per stage — `descent.update` and the camp receive `min(1, scroll / descentShare)`), the remaining 4/13 are the transition, driven by `transP` 0..1 through the same smoothed scroll. `enabled: false` restores the 9-viewport page.

What happens over the tail, all keyed in config:
- **Camera** — `lookDrop` (0 → −190) and `camDrop` (0 → −40) are added to the descent choreography and `zoomMul` (1 → 1.08) multiplies inside the responsive clamp: the mountain leaves through the top at constant size (a dive, not a retreat). Drag and idle rotation untouched.
- **Cloud quads** — `uDusk` / `uDuskBand` / `uDuskColor` / `uDuskThin` on the cloud shader: plates cool to `duskColor` from the bottom up (world-y band that climbs with dusk; the last 45 % takes every plate) and thin out by `duskThin` so what lies beneath shows through. At dusk 0 the shader is pixel-identical to before.
- **Cloud sea** — `uDusk` (cooling), `uHole` + `uHoleCenter` (a noise-eroded rupture opening on the camera's side of the axis, `seaHoleOffset` toward the camera, radius 0 → 230, centre re-computed per frame from the total orbit angle) and `uCollar` (the sea stays solid within `seaCollar` of the axis, so the mountain's cut base is never exposed).
- **Chasm edge** — `split1.png` on a 360 × 214 quad in the cloud rig (so it faces the camera and turns with a drag), half-way between the summit axis and the camera, rising from y −220 to −100 (`split.rise`), fading in 0.35 → 0.65 (`fadeIn`), side edges feathered. Drawn without depth test (it must show through the rupture; it only appears once the mountain has left the frame). Cold relight (`rimColor`, `rimStrength`) applied where the plate is already lit, so the ridge crest catches the abyss light and the body stays dark.
- **Abyss light** — placeholder: an additive Gaussian disc (`abyss.color` / `coreColor`) farther back and lower than the chasm edge, rising more slowly (parallax: clouds / edge / light part from each other), drawn right after the edge so the light sits inside the bowl. Plus `AbyssFloor`, a camera-following dark disc 350 below the camera — the sky cylinder is open at the bottom and the white clear colour showed.
- **Callouts and hero statement** dissolve over the first 30–40 % of the tail (`labelFade`, `heroTextFade` — CSS opacity on `#route-labels`, `heroText.setFade()`); both belong to the mountain world.
- Route: untouched (ends at END as before); the conduit stays as the mountain rises out.

Not done here by design: the canyon video, the ocean scene, a 3-D split bowl (the quad is a stand-in; a GLB would replace `split` in the same slot). Previews: `previews/abyss/transition-{start,mid,075,end}.png` (1440 × 900, orbit frozen). Debug readout now shows `descent` and `abyss` progress.
