...
- **Camera** — translates DOWN only (`camera.drop`, position and look-at by the same amount) plus `zoomMul` 1 → 1.06 inside the responsive clamp. The hero view angle never changes, so the mountain slides out through the top at its size (a small step back, no shrink) and its cut base is never seen from below. Drag / idle rotation untouched.
- **Chasm edge** — `split1.png` as a billboard 80 units in front of the camera, 1.12 frame widths wide, drawn in front of the cloud plates (renderOrder 1.5). Its crest is choreographed in frame heights (`split.rimY`, 0 = centre, +0.5 = top edge) and is additionally clamped to stay `rimAboveBase` above the lowest projected point of the mountain's footprint (`baseY`, `baseRadius`), so once the cloud sea is gone the rocks hide the base as it rises. The body under the crest thins to `bodyOpacity` toward its lower fade (the canyon reads through it); `crestLight` relights the already-lit ridge. Ends at 40 % opacity as framing.
- **Canyon video** — `assets/videos/canyon-a.mp4` on a `VideoTexture` billboard 150 units in front of the camera (deeper than the edge → slower parallax), 1.25 frame widths, feathered border, surfacing from `hazeColor` as `video.opacity` rises (0.02 → 0.5); its own `frameY` path rises more slowly than the crest. Muted, looped, inline; plays only while the transition is active.
- **Clouds** — the plates cool to `duskColor` from the bottom up (`uDusk`, `uDuskBand`, `uDuskColor`, `uDuskThin` on the cloud shader; rig-relative height) and thin by `duskThin` — wisps stay around the mountain, the lower wall dissolves. The rig follows `cloudFollow` = 0.5 of the camera drop. The world-fixed cloud sea fades out (`seaFade`) before the camera passes its level. At dusk 0 the cloud shader is pixel-identical to before.
- **Callouts and hero statement** dissolve over the first 30–40 % of the tail (`labelFade`, `heroTextFade`).
- `AbyssFloor`: a camera-following dark disc far below (the sky cylinder is open at the bottom).
- Route, mountain, sky, nav: untouched.

Timing (transition progress): 0–0.25 crest rises, clouds start cooling, video faint · 0.25–0.55 video clear through the opening, mountain starts up · 0.55–0.8 overlap (storyboard 02) · 0.8–1 mountain out through the top, canyon dominant (03). Previews: `previews/abyss/01-mountain-dominant.png` (t 0.2), `02-overlap.png` (t 0.65), `03-ocean-dominant.png` (t 1), 1440 × 900, orbit frozen. Debug readout shows `descent` and `abyss` progress.
