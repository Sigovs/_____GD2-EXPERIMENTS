...
- **Camera** — does NOT move. The whole hero picture (mountain, baby peaks, clouds, sea, hero text, projected labels) slides up as one image through a **lens shift** of the projection (`camera.shift` in frame heights → `projectionMatrix.elements[9]`, `abyss.applyShift()`), plus `zoomMul` 1 → 1.15 inside the responsive clamp. Nothing new is ever revealed on the models — no undersides, no cut bases, on any aspect. Drag / idle rotation untouched.
